    ...
    std::string strPostData/*=""*/)
{
    ...
    string re = UtfToGbk(strRet.data());
    return re;// std::move(strRet);
}

// 解析Json数据 [11/8/2016/shike]
string CWininetHttp::ParseJsonInfo(const std::string& strJsonInfo)
{
    //有应用解析json 不再HTTP方法i中使用
    //neb::CJsonObject oJson(strJsonInfo);

    //std::cout << " requet data=>\n" << oJson.ToFormattedString();
